...
import { getEmbeddingModels, getGroups, getCurrentUserRoles } from '../services/api';
...
const DataCollections = () => {
            ...
            render: (text, record) => (
                <Space size="middle">
                    ...
                    {(activeTab === 'platform' || activeTab === 'group') && record.group_id && (
                        <Tag color="purple" icon={<GroupOutlined />}>
                            {groupsMap[record.group_id] || record.group_name || 'Group'}
                        </Tag>
                    )}
                    {activeTab === 'platform' && !record.group_id && (
                        <Tag color="blue" icon={<UserOutlined />}>Personal</Tag>
                    )}
                </Space>
            ),
        },
    ...
    const refreshCollections = async () => {
        ...
        try {
            // Use show_all for platform tab (admin only)
            const showAll = activeTab === 'platform' && isAdmin;
            const url = `${API_BASE_URL}/data-collections/collections/${showAll ? '?show_all=true' : ''}`;
        ...
        const checkAdminStatus = async () => {
            if (initialized && keycloak.authenticated) {
                try {
                    ...
                    setIsAdmin(rolesData.is_admin === true);
                } catch (err) {
                ...
                items={[
                    {
                        ...
                        children: (
                            <Card className="collections-card" bordered={false}>
                                <Table
                                    columns={columns}
                                    dataSource={collections.filter(c => !c.group_id)}
                                    rowKey="id"
                                    ...
                                    locale={{
                                        emptyText: (
                                            <div className="empty-table">
                                                ...
                                                <div>No personal collections found</div>
                                ...
                                <Table
                                    ...
                                    locale={{
                                        emptyText: (
                                            <div className="empty-table">
                                                <FileSearchOutlined style={{ fontSize: 48, color: '#bfbfbf', marginBottom: 16 }} />
                                                <div>No group collections found</div>
                                            </div>
                                        )
                                    }}
                                />
